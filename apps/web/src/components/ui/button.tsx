...
function Button({ className, variant, size, ...props }: ButtonProps & ButtonPrimitive.Props) {
  return (
    <ButtonPrimitive className={cn(buttonVariants({ className, size, variant }))} {...props} />
  );
}
...
function LinkButton({
  ...
  size,
  ...props
}: ButtonProps & LinkProps & React.ComponentProps<"a">) {
